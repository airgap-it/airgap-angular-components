...
export class CryptoOutput extends RegistryItem {
  ...
  constructor(private scriptExpressions: ScriptExpression[], private cryptoKey: CryptoHDKey | CryptoECKey | MultiKey) {
    super()
  }
  ...
  public static fromDataItem = (dataItem: DataItem) => {
    ...
    const isMultiKey =
      seLength > 0 &&
      (scriptExpressions[seLength - 1].getExpression() === ScriptExpressions.MULTISIG.getExpression() ||
        scriptExpressions[seLength - 1].getExpression() === ScriptExpressions.SORTED_MULTISIG.getExpression())
